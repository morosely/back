...
public class GoodsModel extends CommonModel{
    ...
    //香港的品类级别4,5级允许相同的品类编码。商品的品类编码一定是品类的末级（第5级）
    public String getCategoryKeyValue() {
		return "'"+this.categoryCode+"','"+this.erpCode+"',"+this.entId+","+ 5;
	}
    
    public String getGoodsCategoryKeyValue() {
    	return "'"+this.goodsCode+"','"+this.categoryCode+"','"+this.erpCode+"',"+this.entId;
    }

    public String getBrandCodeKeyValue() {
        if(this.brandCode  != null){
            this.brandCode = this.brandCode.replaceAll("\'","\\\\'");
        }
        String brandKey = "'" + this.brandCode + "','" + this.erpCode + "'," + this.entId;
        return brandKey;
    }
