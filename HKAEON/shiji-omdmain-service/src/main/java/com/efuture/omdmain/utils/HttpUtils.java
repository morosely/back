...
public class HttpUtils {
	...
	public static String onPost(String url, ServiceSession session, String jsonString) {
		logger.info("------>>>>> 1.url : " + url + " 2.jsonString: " + jsonString);
		String traceId = "";
		...
		try {
			...
			String response = RestBaseUtils.onHttpPost(traceId1, connectTimeOut, readTimeOut, null, url, paramsMap,
					jsonString, String.class, null, null);

			// 重试一次
			if (response == null) {
				response = RestBaseUtils.onHttpPost(traceId1, connectTimeOut, readTimeOut, null, url, paramsMap,
						jsonString, String.class, null, null);
			}
	...
	public static JSONArray doPost(String url, ServiceSession session, String json_param, String tableName) {
		...
		try {
			...
			// 记录日志
			HttpLog.log(traceId, url, json_param, response, end - start);
			return array;

		} catch (Exception e) {
			HttpLog.log(traceId, url, json_param, response, -1);
			logger.error("{traceId:" + traceId + "}", e);
