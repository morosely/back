...
public class PosSaleGoodsOutModel {
	/**
	 * 货号
	 */
	...
	@Transient
	@ReferQuery(table = "brandInfo", query = "{brandCode:'$brandCode', entId:'$entId' , erpCode:'$erpCode'}", set = "{brandName:'brandName'}", operationFlags = {OperationFlag.afterQuery })
	...
	@Transient
	@ReferQuery(table = "category", query = "{categoryId:'$categoryId', entId:'$entId' , erpCode:'$erpCode'}", set = "{categoryName:'categoryName'}", operationFlags = {OperationFlag.afterQuery })
	 ...
	 * 净重
	 */
	@ReferQuery(table = "goods", query = "{sgid:'$sgid'}", set = "{nweight:'nweight',qaDays:'qaDays',qaDaysUnit:'qaDaysUnit',orderNum:'orderNum'}", operationFlags = {OperationFlag.afterQuery })
	 ...
	 */
	@Transient
	@ReferQuery(table = "shop", query = "{shopId:'$shopId'}", set = "{shopName:'shopName'}", operationFlags = {
			OperationFlag.afterQuery })
	 ...
	 */
	@Transient
	@ReferQuery(table = "goodsMoreBarCode", query = "{entId:'$entId', erpCode:'$erpCode', barNo:'$barNo'}", set = "{sortLevel:'sortLevel'}", operationFlags = {
			OperationFlag.afterQuery })
	 ...
	 */
	@Transient
	@ReferQuery(table = "vender", query = "{vid:'$vid', entId:'$entId' , erpCode:'$erpCode'}", set = "{venderName:'venderName'}", operationFlags = {
			OperationFlag.afterQuery })
	 ...
	 *
	 * @return goodsStatus - 商品状态 0 待启用/1新品试销/
	 *         2新品评估/3正常/4暂停订货/5换季处理/6暂停经营/7停止销售/8待清退/9已清退/10暂停订补
	 */
